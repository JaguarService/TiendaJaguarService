...
function actualizarTotalCarrito(){
   ...
   for(var i=0; i < carritoItems.length;i++){
    ...
    var precio = parseFloat(precioElemento.innerText.replace('$','').replace(',','').replace('MXN',''));
    console.log(precio);
   ...
   document.getElementsByClassName('carrito-precio-total')[0].innerText = '$' + total.toLocaleString("es") + ' MXN';
   document.getElementById('carrito-precio-total-otro-lado').innerText = '$' + total.toLocaleString("es") + ' MXN';
}
...
function actualizarSubtotal(item) {
  ...
  var precioUnitario = parseFloat(item.getElementsByClassName('carrito-item-precio')[0].innerText.replace('$', '').replace(',', '').replace('MXN', ''));
...
function agregarItemAlCarrito(titulo, precio, imagenSrc) {
  ...
  if (itemExistente) {
    ...
    var precioUnitario = parseFloat(precio.replace('$', '').replace(',', '').replace('MXN', ''));
    ...
    copiarAlPortapapeles();
      ...
      function pagarClicked(event) {
        ...
        for (var i = 0; i < carritoItems.length; i++) {
          ...
          var cantidad = item.getElementsByClassName('carrito-item-cantidad')[0].value;
          var subtotal = parseFloat(precio.replace('$', '').replace(',', '').replace('MXN', '')) * cantidad;
        ...
        window.location.href = 'f_mxn.html?texto=' + encodeURIComponent(texto);
      }
