...
function actualizarTotalCarrito() {
    ...
    for (var i = 0; i < carritoItems.length; i++) {
      var item = carritoItems[i];
      var subtotal = parseFloat(item.getElementsByClassName('carrito-item-subtotal')[0].innerText.replace('$', '').replace(',', '').replace('USD', ''));
      total += subtotal;
    }
  
    total = Math.round(total * 100) / 100;
    document.getElementsByClassName('carrito-precio-total')[0].innerText = '$' + total.toLocaleString("es") + 'SD';
  }
...
function agregarAlCarritoClicked(event) {
    var botonAgregar = event.target;
    var producto = botonAgregar.parentNode; // <div class="item">
    var titulo = producto.childNodes[0].innerText; // <span class="titulo-item">
    var imagen = producto.childNodes[1]; // <img src="...">
    var precioElement = producto.querySelector('.precio-item'); // <span class="precio-item">
    var precio = precioElement ? precioElement.innerText : undefined;
    ...
    var precioNumerico = parseFloat(precio.replace('$', '').replace(',', '').replace('USD', ''));
    ...
    agregarItemAlCarrito(titulo, precio, imagen.src, cantidad, subtotal);
     //hacemos visible el carrito cuando agrega por primera vez
     hacerVisibleCarrito();
  
  }

function agregarItemAlCarrito(titulo, precio, imagenSrc, subtotal){
    ...
    <div class="carrito-item">
        ...
        <div class="carrito-item-detalles">
            ...
            <span class="carrito-item-subtotal">$${subtotal.toLocaleString("es")}</span>
            </div>
